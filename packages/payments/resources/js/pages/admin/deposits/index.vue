<template>
  <v-container>
    ...
    >
      ...
      <template v-slot:item.method.name="{ item }">
        
        {{ (item.external_id && item.external_id.includes('DEPOSIT IN GAMES WITH TXN')) ? 'Deposited from Network' : item.method.name }}
      ...
      <template v-slot:item.actions="{ item }">
        <deposit-menu :id="item.id" :item="item" small />
      </template>
    </data-table>
  </v-container>
</template>
