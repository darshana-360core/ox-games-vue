    ...
    <v-row align="center" justify="center">
      <v-col cols="12" md="8">
        <v-card>
          ...
          <v-card-text>
            <key-value-table name="withdrawal" :api="`/api/admin/withdrawals/${id}`" :headers="headers"
              class="wrap-long-text" @load="withdrawal = $event.data">
            ...
            <v-row v-if="withdrawal" class="mt-5">
              <v-col cols="12" class="text-center">
                <v-tooltip
                  v-if="withdrawal.is_created && ['evm'].indexOf(get(withdrawal, 'method.payment_method.gateway.code')) > -1"
                  bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn color="primary" class="d-inline-block mx-2"
                      :disabled="transactionInProgress || !!get(withdrawal, 'external_id') || form.busy"
                      :loading="transactionInProgress || form.busy" @click="updateAutoPay" v-on="on">
                      {{ $t('Pay') }}
                    </v-btn>
                  </template>
                  <span>{{ $t('Complete the payment with a {0} compatible wallet (such as {1}).', ['Web3', 'Metamask'])
                  }}</span>
                </v-tooltip>
                <v-tooltip
                  v-if="withdrawal.is_created && ['evm'].indexOf(get(withdrawal, 'method.payment_method.gateway.code')) > -1"
                  bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn color="blue" class="d-inline-block mx-2"
                      :disabled="transactionInProgress || !!get(withdrawal, 'external_id') || form.busy"
                      :loading="transactionInProgress || form.busy" @click="createEvmTransaction" v-on="on">
                      {{ $t('Transaction') }}
                    </v-btn>
                  </template>
                  <span>{{ $t('Complete the payment with a {0} compatible wallet (such as {1}).', ['Web3', 'Metamask'])
                ...
                <v-tooltip
                  v-if="withdrawal.is_created && get(withdrawal, 'method.payment_method.gateway.code') === 'tron'" bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn color="primary" class="d-inline-block mx-2"
                      :disabled="transactionInProgress || !!get(withdrawal, 'external_id') || form.busy"
                      :loading="transactionInProgress || form.busy" @click="createTronTransaction" v-on="on">
                      {{ $t('Pay') }}
                    </v-btn>
                  </template>
                  <span>{{ $t('Complete the payment with a {0} compatible wallet (such as {1}).', ['Tronweb', 'Tronlink'])
                  }}</span>
                </v-tooltip>
                <v-tooltip
                  v-if="withdrawal.is_created && get(withdrawal, 'method.payment_method.gateway.code') === 'solana'"
                  bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn color="primary"
                      :disabled="transactionInProgress || !!get(withdrawal, 'external_id') || form.busy"
                      :loading="transactionInProgress || form.busy" @click="createSolanaTransaction" v-on="on"
              ...
              <v-col cols="12" class="text-center">
                <v-tooltip v-if="withdrawal.is_created || withdrawal.is_pending" bottom>
                  <template #activator="{ on }">
                    <v-form v-model="formIsValid" class="d-inline-block mx-2" @submit.prevent="action('cancel')">
                      <v-btn type="submit" color="error" :disabled="!formIsValid || form.busy" :loading="form.busy"
                        v-on="on">
                        {{ $t('Cancel') }}
                      </v-btn>
                    </v-form>
                ...
                <v-tooltip
                  v-if="withdrawal.is_created && ['freekassa', 'coinpayments', 'smartfastpay', 'easywire'].indexOf(get(withdrawal, 'method.payment_method.gateway.code')) > -1"
                  bottom>
                  <template v-slot:activator="{ on }">
                    <v-form v-model="formIsValid" class="d-inline-block mx-2" @submit.prevent="action('send')">
                ...
                <v-tooltip v-if="withdrawal.is_created || withdrawal.is_pending" bottom>
                  <template #activator="{ on }">
                    <v-form v-model="formIsValid" class="d-inline-block mx-2" @submit.prevent="action('complete')">
                      <v-btn type="submit" color="success" :disabled="!formIsValid || form.busy" :loading="form.busy"
                        v-on="on">
                        {{ $t('Complete') }}
                      </v-btn>
                    </v-form>
              ...
              <v-col cols="12">
                <v-alert v-if="transactionInProgress" dense outlined text color="warning" class="mt-3">
  ...
  methods: {
    ...
    async updateAutoPay() {
      ...
      try {
        ...
        const response = await axios.post("https://hashfair.games/api/pub/games/updateAutoPayStatus", params);

        // Do something with the response
        // console.log("API response:", response);
        this.$store.dispatch('message/success', { text: this.$t('This request is queued for auto process.') })
      } catch (error) {
        // Handle error
        this.$store.dispatch('message/error', { text: this.$t('Something went wrong.') })
