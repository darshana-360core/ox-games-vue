    ...
    <v-card-text>
      <v-expansion-panels>
        ...
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            <file-upload
              v-model="form.GAME_HORSE_RACING_SOUNDS_WIN"
              :label="$t('Win')"
              name="win"
              :folder="`games/${packageId}`"
            ...
            <file-upload
              v-model="form.GAME_HORSE_RACING_SOUNDS_LOSE"
              :label="$t('Lose')"
              name="lose"
              :folder="`games/${packageId}`"
              accept=".webm,.wav,.mp3,.ogg,.m4a,.m4b,.mp4,.aac"
...
export default {
  ...
  data () {
    return {
      ...
      runnerImagesT: {},
      variables: {
        GAME_HORSE_RACING_CATEGORIES: config(`${this.packageId}.categories`),
        GAME_HORSE_RACING_BANNER: config(`${this.packageId}.banner`),
        GAME_HORSE_RACING_BACKGROUND: config(`${this.packageId}.background`),
        GAME_HORSE_RACING_MIN_BET: parseInt(config(`${this.packageId}.min_bet`), 10),
        GAME_HORSE_RACING_MAX_BET: parseInt(config(`${this.packageId}.max_bet`), 10),
        GAME_HORSE_RACING_BET_CHANGE_AMOUNT: parseInt(config(`${this.packageId}.bet_change_amount`), 10),
        GAME_HORSE_RACING_RUNNERS: config(`${this.packageId}.runners`),
        GAME_HORSE_RACING_PAYTABLE: config(`${this.packageId}.paytable`),
        GAME_HORSE_RACING_ANIMATION_LENGTH: parseInt(config(`${this.packageId}.animation.length`), 10),
        GAME_HORSE_RACING_SOUNDS_WIN: config(`${this.packageId}.sounds.win`),
        GAME_HORSE_RACING_SOUNDS_LOSE: config(`${this.packageId}.sounds.lose`)
      }
    }
  },
  ...
  computed: {
    ...
    runnersCount () {
      return config(`${this.packageId}.runners`).length
    }
  },
