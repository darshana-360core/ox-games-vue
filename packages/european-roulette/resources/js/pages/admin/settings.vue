      ...
      <v-expansion-panels>
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            ...
            <v-switch
              v-model="form.GAME_EUROPEAN_ROULETTE_AUTO_PLAY_ENABLED"
              :label="$t('Auto play')"
              color="primary"
              false-value="false"
              true-value="true"
            ...
            <v-text-field
              v-show="form.GAME_EUROPEAN_ROULETTE_AUTO_PLAY_ENABLED === 'true'"
              v-model.number="form.GAME_EUROPEAN_ROULETTE_AUTO_PLAY_INTERVAL"
              :label="$t('Auto play interval')"
              :rules="[validationPositiveInteger]"
              :error="form.errors.has('GAME_EUROPEAN_ROULETTE_AUTO_PLAY_INTERVAL')"
              :error-messages="form.errors.get('GAME_EUROPEAN_ROULETTE_AUTO_PLAY_INTERVAL')"
              outlined
              :suffix="$t('seconds')"
              @keydown="clearFormErrors($event, 'GAME_EUROPEAN_ROULETTE_AUTO_PLAY_INTERVAL')"
            />
          </v-expansion-panel-content>
        </v-expansion-panel>
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            <file-upload
              v-model="form.GAME_EUROPEAN_ROULETTE_SOUNDS_WIN"
              :label="$t('Win')"
              name="win"
              :folder="`games/${packageId}`"
            ...
            <file-upload
              v-model="form.GAME_EUROPEAN_ROULETTE_SOUNDS_LOSE"
              :label="$t('Lose')"
              name="lose"
              :folder="`games/${packageId}`"
              accept=".webm,.wav,.mp3,.ogg,.m4a,.m4b,.mp4,.aac"
...
export default {
  ...
  data () {
    return {
      variables: {
        GAME_EUROPEAN_ROULETTE_CATEGORIES: config(`${this.packageId}.categories`),
        GAME_EUROPEAN_ROULETTE_BANNER: config(`${this.packageId}.banner`),
        GAME_EUROPEAN_ROULETTE_BACKGROUND: config(`${this.packageId}.background`),
        GAME_EUROPEAN_ROULETTE_MIN_BET: config(`${this.packageId}.min_bet`),
        GAME_EUROPEAN_ROULETTE_MAX_BET: config(`${this.packageId}.max_bet`),
        GAME_EUROPEAN_ROULETTE_BET_CHANGE_AMOUNT: config(`${this.packageId}.bet_change_amount`),
        GAME_EUROPEAN_ROULETTE_DEFAULT_BET_AMOUNT: config(`${this.packageId}.default_bet_amount`),
        GAME_EUROPEAN_ROULETTE_AUTO_PLAY_ENABLED: '' + config(`${this.packageId}.auto_play.enabled`),
        GAME_EUROPEAN_ROULETTE_AUTO_PLAY_INTERVAL: config(`${this.packageId}.auto_play.interval`),
        GAME_EUROPEAN_ROULETTE_SOUNDS_WIN: config(`${this.packageId}.sounds.win`),
        GAME_EUROPEAN_ROULETTE_SOUNDS_LOSE: config(`${this.packageId}.sounds.lose`)
      }
    }
  },
