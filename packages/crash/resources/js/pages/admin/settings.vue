    ...
    <v-card-text v-if="pusherEnabled">
      ...
      <v-expansion-panels>
        ...
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            <file-upload
              v-model="form.GAME_CRASH_SOUNDS_WIN"
              :label="$t('Win')"
              name="win"
              :folder="`games/${packageId}`"
            ...
            <file-upload
              v-model="form.GAME_CRASH_SOUNDS_CRASH"
              :label="$t('Crash')"
              name="lose"
              :folder="`games/${packageId}`"
              accept=".webm,.wav,.mp3,.ogg,.m4a,.m4b,.mp4,.aac"
...
export default {
  ...
  data () {
    return {
      variables: {
        GAME_CRASH_CATEGORIES: config(`${this.packageId}.categories`),
        GAME_CRASH_BANNER: config(`${this.packageId}.banner`),
        GAME_CRASH_BACKGROUND: config(`${this.packageId}.background`),
        GAME_CRASH_MIN_BET: config(`${this.packageId}.min_bet`),
        GAME_CRASH_MAX_BET: config(`${this.packageId}.max_bet`),
        GAME_CRASH_BET_CHANGE_AMOUNT: config(`${this.packageId}.bet_change_amount`),
        GAME_CRASH_DEFAULT_BET_AMOUNT: config(`${this.packageId}.default_bet_amount`),
        GAME_CRASH_DURATION: config(`${this.packageId}.duration`),
        GAME_CRASH_INTERVAL: config(`${this.packageId}.interval`),
        GAME_CRASH_BASE_NUMBER: config(`${this.packageId}.base_number`),
        GAME_CRASH_BOTS_ENABLED: '' + config(`${this.packageId}.bots_enabled`),
        GAME_CRASH_PAYOUT_INTERVALS: config(`${this.packageId}.payout_intervals`),
        GAME_CRASH_ANIMATION: config(`${this.packageId}.animation`),
        GAME_CRASH_SOUNDS_WIN: config(`${this.packageId}.sounds.win`),
        GAME_CRASH_SOUNDS_CRASH: config(`${this.packageId}.sounds.crash`)
      }
    }
  },
