<template>
  <v-card flat>
    <v-card-text>
      <v-expansion-panels>
        <v-expansion-panel v-for="(variation, variationIndex) in form.GAME_LUCKY_WHEEL_VARIATIONS" :key="variationIndex">
            ...
            <v-expansion-panels>
              <v-expansion-panel>
                ...
                <v-expansion-panel-content>
                  ...
                  <v-switch
                    v-model="form.GAME_LUCKY_WHEEL_VARIATIONS[variationIndex].auto_play.enabled"
                    :label="$t('Auto play')"
                    color="primary"
                    :false-value="false"
                    :true-value="true"
                  ...
                  <v-text-field
                    v-show="form.GAME_LUCKY_WHEEL_VARIATIONS[variationIndex].auto_play.enabled"
                    v-model.number="form.GAME_LUCKY_WHEEL_VARIATIONS[variationIndex].auto_play.interval"
              ...
              <v-expansion-panel>
                ...
                <v-expansion-panel-content>
                  <file-upload
                    v-model="form.GAME_LUCKY_WHEEL_VARIATIONS[variationIndex].sounds.win"
                    :label="$t('Win')"
                    name="win"
                    :folder="`games/${packageId}/${variation.id}`"
                    accept=".webm,.wav,.mp3,.ogg,.m4a,.m4b,.mp4,.aac"
                    :clearable="true"
                  />

                  <file-upload
                    v-model="form.GAME_LUCKY_WHEEL_VARIATIONS[variationIndex].sounds.lose"
                    :label="$t('Lose')"
                    name="lose"
                    :folder="`games/${packageId}/${variation.id}`"
                    accept=".webm,.wav,.mp3,.ogg,.m4a,.m4b,.mp4,.aac"
...
export default {
  ...
  created () {
    const variations = config(`${this.packageId}.variations`).map((v, i) => {
      if (typeof v.auto_play === 'undefined') {
        v.auto_play = { enabled: true, interval: 8 }
      }
    ...
    this.$emit('input', { GAME_LUCKY_WHEEL_VARIATIONS: variations })
  },
