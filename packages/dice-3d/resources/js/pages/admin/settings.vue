      ...
      <v-expansion-panels>
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            ...
            <v-switch
              v-model="form.GAME_DICE_3D_AUTO_PLAY_ENABLED"
              :label="$t('Auto play')"
              color="primary"
              false-value="false"
              true-value="true"
            ...
            <v-text-field
              v-show="form.GAME_DICE_3D_AUTO_PLAY_ENABLED === 'true'"
              v-model.number="form.GAME_DICE_3D_AUTO_PLAY_INTERVAL"
              :label="$t('Auto play interval')"
              :rules="[validationPositiveInteger]"
              :error="form.errors.has('GAME_DICE_3D_AUTO_PLAY_INTERVAL')"
              :error-messages="form.errors.get('GAME_DICE_3D_AUTO_PLAY_INTERVAL')"
              outlined
              :suffix="$t('seconds')"
              @keydown="clearFormErrors($event, 'GAME_DICE_3D_AUTO_PLAY_INTERVAL')"
            />
        ...
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            <file-upload
              v-model="form.GAME_DICE_3D_SOUNDS_WIN"
              :label="$t('Win')"
              name="win"
              :folder="`games/${packageId}`"
            ...
            <file-upload
              v-model="form.GAME_DICE_3D_SOUNDS_LOSE"
              :label="$t('Lose')"
              name="lose"
              :folder="`games/${packageId}`"
              accept=".webm,.wav,.mp3,.ogg,.m4a,.m4b,.mp4,.aac"
...
export default {
  ...
  data () {
    return {
      ...
      variables: {
        GAME_DICE_3D_CATEGORIES: config(`${this.packageId}.categories`),
        GAME_DICE_3D_BANNER: config(`${this.packageId}.banner`),
        GAME_DICE_3D_BACKGROUND: config(`${this.packageId}.background`),
        GAME_DICE_3D_MIN_BET: parseInt(config(`${this.packageId}.min_bet`), 10),
        GAME_DICE_3D_MAX_BET: parseInt(config(`${this.packageId}.max_bet`), 10),
        GAME_DICE_3D_BET_CHANGE_AMOUNT: parseInt(config(`${this.packageId}.bet_change_amount`), 10),
        GAME_DICE_3D_DEFAULT_BET_AMOUNT: parseInt(config(`${this.packageId}.default_bet_amount`), 10),
        GAME_DICE_3D_MIN_WIN_CHANCE: parseFloat(config(`${this.packageId}.min_win_chance`)),
        GAME_DICE_3D_MAX_WIN_CHANCE: parseFloat(config(`${this.packageId}.max_win_chance`)),
        GAME_DICE_3D_HOUSE_EDGE: parseFloat(config(`${this.packageId}.house_edge`)),
        GAME_DICE_3D_AUTO_PLAY_ENABLED: '' + config(`${this.packageId}.auto_play.enabled`),
        GAME_DICE_3D_AUTO_PLAY_INTERVAL: config(`${this.packageId}.auto_play.interval`),
        GAME_DICE_3D_DICE: config(`${this.packageId}.dice`),
        GAME_DICE_3D_FILL_COLOR: config(`${this.packageId}.fill_color`),
        GAME_DICE_3D_FONT_COLOR: config(`${this.packageId}.font_color`),
        GAME_DICE_3D_SOUNDS_WIN: config(`${this.packageId}.sounds.win`),
        GAME_DICE_3D_SOUNDS_LOSE: config(`${this.packageId}.sounds.lose`)
      }
    }
  },
