      ...
      if (y > body.last_y) {
        ...
        if (x === body.pegs[y]) {
          if (body.path[y] === 0 && (body.velocity.x > 0)) {
            Body.setVelocity(body, { x: (body.path[y] ? 1 : -1) * (0.15 + (body.position.x > bodyPeg.position.x ? ((body.position.x - bodyPeg.position.x) / 2) : 0) + 0.5 * Math.random()), y: (vy < 1.5 ? 1.5 : vy) })
          } else if (body.path[y] === 1 && (body.velocity.x < 0)) {
            Body.setVelocity(body, { x: (body.path[y] ? 1 : -1) * (0.15 + (body.position.x < bodyPeg.position.x ? ((bodyPeg.position.x - body.position.x) / 2) : 0) + 0.5 * Math.random()), y: (vy < 1.5 ? 1.5 : vy) })
          } else if (body.path[y] === 0 && body.velocity.x < -2) {
            Body.setVelocity(body, { x: -2, y: vy })
          } else if (body.path[y] === 1 && body.velocity.x > 2) {
            Body.setVelocity(body, { x: 2, y: vy })
          }
        }
        if (x > body.pegs[y] && body.velocity.x > 0) {
          Body.setVelocity(body, { x: -0.1, y: vy < 0.1 ? 0.15 : vy })
