      ...
      <v-expansion-panels>
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            <v-text-field
              v-model.number="form.RAFFLE_DISPLAY_COUNT_HOME_PAGE"
              :label="$t('Max number of raffles on home page')"
              :rules="[validationPositiveInteger]"
              :error="form.errors.has('RAFFLE_DISPLAY_COUNT_HOME_PAGE')"
              :error-messages="form.errors.get('RAFFLE_DISPLAY_COUNT_HOME_PAGE')"
              outlined
              persistent-hint
              @keydown="clearFormErrors($event,'RAFFLE_DISPLAY_COUNT_HOME_PAGE')"
            />
...
export default {
  ...
  data () {
    return {
      variables: {
        RAFFLE_DISPLAY_COUNT_HOME_PAGE: config(`${this.packageId}.display_count_home_page`),
        RAFFLE_DISPLAY_COUNT: config(`${this.packageId}.display_count`),
        RAFFLE_LOAD_COUNT: config(`${this.packageId}.load_count`),
        RAFFLE_BOTS_FREQUENCY: config(`${this.packageId}.bots.frequency`),
        RAFFLE_BOTS_MIN_COUNT: config(`${this.packageId}.bots.min_count`),
        RAFFLE_BOTS_MAX_COUNT: config(`${this.packageId}.bots.max_count`),
        RAFFLE_BOTS_MIN_TICKETS: config(`${this.packageId}.bots.min_tickets`),
        RAFFLE_BOTS_MAX_TICKETS: config(`${this.packageId}.bots.max_tickets`)
      }
    }
  },
