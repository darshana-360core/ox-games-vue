      ...
      <v-expansion-panels>
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            ...
            <v-switch
              v-model="form.GAME_SLOTS_3D_AUTO_PLAY_ENABLED"
              :label="$t('Auto play')"
              color="primary"
              false-value="false"
              true-value="true"
            ...
            <v-text-field
              v-show="form.GAME_SLOTS_3D_AUTO_PLAY_ENABLED === 'true'"
              v-model.number="form.GAME_SLOTS_3D_AUTO_PLAY_INTERVAL"
              :label="$t('Auto play interval')"
              :rules="[validationPositiveInteger]"
              :error="form.errors.has('GAME_SLOTS_3D_AUTO_PLAY_INTERVAL')"
              :error-messages="form.errors.get('GAME_SLOTS_3D_AUTO_PLAY_INTERVAL')"
              outlined
              :suffix="$t('seconds')"
              @keydown="clearFormErrors($event, 'GAME_SLOTS_3D_AUTO_PLAY_INTERVAL')"
            />
          </v-expansion-panel-content>
        </v-expansion-panel>
        <v-expansion-panel>
        ...
        <v-expansion-panel>
          ...
          <v-expansion-panel-content>
            <file-upload
              v-model="form.GAME_SLOTS_3D_SOUNDS_WIN"
              :label="$t('Win')"
              name="win"
              :folder="`games/${packageId}`"
            ...
            <file-upload
              v-model="form.GAME_SLOTS_3D_SOUNDS_LOSE"
              :label="$t('Lose')"
              name="lose"
              :folder="`games/${packageId}`"
              accept=".webm,.wav,.mp3,.ogg,.m4a,.m4b,.mp4,.aac"
...
export default {
  ...
  data () {
    return {
      variables: {
        GAME_SLOTS_3D_CATEGORIES: config(`${this.packageId}.categories`),
        GAME_SLOTS_3D_BANNER: config(`${this.packageId}.banner`),
        GAME_SLOTS_3D_BACKGROUND_SKYBOX: config(`${this.packageId}.background_skybox`),
        GAME_SLOTS_3D_BACKGROUND_SKYBOX_ROTATE: config(`${this.packageId}.background_skybox_rotate`),
        GAME_SLOTS_3D_BACKGROUND_SKYBOX_DEEP: config(`${this.packageId}.background_skybox_deep`),
        GAME_SLOTS_3D_TEXTURE: config(`${this.packageId}.texture`),
        GAME_SLOTS_3D_TABLE_TEXTURE: config(`${this.packageId}.table_texture`),
        GAME_SLOTS_3D_TABLE_COLOR: config(`${this.packageId}.table_color`),
        GAME_SLOTS_3D_TABLE_BORDER_COLOR: config(`${this.packageId}.table_border_color`),
        GAME_SLOTS_3D_MIN_BET: parseInt(config(`${this.packageId}.min_bet`), 10),
        GAME_SLOTS_3D_MAX_BET: parseInt(config(`${this.packageId}.max_bet`), 10),
        GAME_SLOTS_3D_BET_CHANGE_AMOUNT: parseInt(config(`${this.packageId}.bet_change_amount`), 10),
        GAME_SLOTS_3D_DEFAULT_BET_AMOUNT: parseInt(config(`${this.packageId}.default_bet_amount`), 10),
        GAME_SLOTS_3D_AUTO_PLAY_ENABLED: '' + config(`${this.packageId}.auto_play.enabled`),
        GAME_SLOTS_3D_AUTO_PLAY_INTERVAL: config(`${this.packageId}.auto_play.interval`),
        GAME_SLOTS_3D_SYMBOLS: config(`${this.packageId}.symbols`),
        GAME_SLOTS_3D_REELS: config(`${this.packageId}.reels`),
        GAME_SLOTS_3D_PAYTABLE: config(`${this.packageId}.paytable`),
        GAME_SLOTS_3D_SOUNDS_WIN: config(`${this.packageId}.sounds.win`),
        GAME_SLOTS_3D_SOUNDS_LOSE: config(`${this.packageId}.sounds.lose`)
      }
    }
  },
