    ...
    <!-- <v-card-subtitle v-if="tippingIsEnabled">
      <v-skeleton-loader type="button" :loading="!user" class="d-flex justify-center">
        <v-form v-if="sendTipFormVisible" v-model="formIsValid" @submit.prevent="sendTip">
          <v-text-field v-model="sendTipForm.amount" dense outlined :full-width="false" :suffix="$t('credits')"
            :rules="[validationPositiveInteger]" :error="sendTipForm.errors.has('amount')"
            :error-messages="sendTipForm.errors.get('amount')" :color="sendTipFormSuccess ? 'success' : ''"
            :success-messages="sendTipFormSuccess ? $t('Your tip was successfully sent') : ''"
            :readonly="sendTipForm.busy || sendTipFormSuccess" class="text-center"
            @keydown="clearFormErrors($event, 'amount', sendTipForm)">
            <template #append-outer>
              <v-btn type="submit" icon :disabled="sendTipForm.busy || !formIsValid || sendTipFormSuccess"
                :loading="sendTipForm.busy" class="mt-n1">
