...
export const getters = {
  ...
  getProviderGamesByCategories: state => categories => {
    return state.providerGames
      .filter(game => categories.indexOf(game.category) > -1 || categories.indexOf(game.id) > -1 || categories.indexOf(`${game.provider.id}-${game.id}`) > -1 || (game.group && categories.indexOf(`${game.provider.id}-${game.group}-${game.id}`) > -1))
      .map(game => {
        if (game.group && categories.indexOf(`${game.provider.id}-${game.group}-${game.id}`) > -1) {
          game.route.query = { group: game.group }
        }
        return game
      })
      .sort((a, b) => {
        return a.categories < b.categories ? -1 : (a.categories > b.categories ? 1 : 0)
      })
  },
